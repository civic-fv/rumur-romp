...
Type
  Seed: 0..255 -- a seed type (unsigned short)
...
Var
  ...
  lock_hist: Array [ pid ] of pid;
  bug: Array [ pid ] of priority; -- where we store first locking order to use as bug seq
  lh_size: priority;  -- start and end of circ buffer
  ls_is_full, bug_is_full: boolean;  -- if we circling yet (aka need to move start/lh_i forward)
...
Begin
  a := seed; b := (seed * 2);
  q := (a | b) & ((!a) | (!b)); -- calculate xor the long way
...
Invariant !(bug_is_full & ls_is_full) |
          Exists b_i: 0..(N-S) Do
