    ...
    for (const Ptr<const SimpleRule>& rule : rules) {
      ...
      *this << indentation() << "/* --- Rules Generated by: `" << rule->name << "` (RuleSet expansions) --- */\n";
      ...
      for (const Sig& sig : sigs) {
        ...
        std::string _guard = ROMP_RULE_GUARD_PREFIX + rule->name + "__" + int_to_hex(sig.index);
        std::string _action = ROMP_RULE_ACTION_PREFIX + rule->name + "__" + int_to_hex(sig.index);
  ...
  void gen_startstate_array(const std::vector<rumur::Ptr<rumur::StartState>>& startstates) {
    ...
    for (const Ptr<const StartState>& startstate : startstates) {
      *this << indentation() << "/* --- StartState Rule(s) generated by: `" << startstate->name << "` (RuleSet expansions) --- */\n";
      ...
      for (auto& sig : sigs) {
        std::string _init = ROMP_PROPERTYRULE_PREFIX + startstate->name + "__" + int_to_hex(sig.index);
        out << indentation() 
