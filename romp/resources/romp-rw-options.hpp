...
void print_help() {
  using namespace std;
  std::cout << "This is a murphi model checker generated from the romp random walker "
               "tool.\n"
    ...
    if ("-h" == argv[i] || "--help" == argv[i]) { // to print help message and ( result of comparison
      ...
      if (i + 1 < argc && '-' != argv[i + 1][0]) { // is it not argv[i+1]
        ...
        if (OPTIONS.threads == 1 && OPTIONS.random_walkers == 1) {
          std::cerr << "invalid argument - Number of threads cannot be specified while using single walk\n" << std::flush;
          exit(EXIT_FAILURE);
          ...
          try {
            ...
            std::cerr
                << "invalid argument : provided walk/search threads was not a number (NaN) !! (for -ptn/--threads flag)\n"
                << std::flush;
            exit(EXIT_FAILURE);
          } catch (std::out_of_range &oor) {
      ...
      } else if ("-w" == argv[i] || "--walk-count" == argv[i])
        if (i + 1 < argc && '-' != argv[i + 1][0]) { // is it not argv[i+1]
          ++i;
          try {
            ...
            std::cerr << "invalid argument : provided number of random walkers was out of range (for -d/--depth flag) must "
      ...
      if (i + 1 < argc && '-' != argv[i + 1][0]) { // is it not argv[i+1]
        ...
        try {
          ...
          std::cerr << "invalid argument : provided liveness count was out of range (for -d/--depth flag) must be unsigned "
        ...
        if (i + 1 < argc && '-' != argv[i + 1][0]) { // is it not argv[i+1]
          ...
          if (i + 1 < argc && '-' != argv[i + 1][0]) { // is it not argv[i+1]
            ...
            try {
              ...
              std::cerr << "invalid argument : provided complete on cover count was not a number (NaN) !! (for "
                           "-w/--walk-count flag)\n"
                        << std::flush;
              exit(EXIT_FAILURE);
            } catch (std::out_of_range &oor) {
              std::cerr << "invalid argument : provided complete on cover count was out of range (for -d/--depth flag) must "
                           "be unsigned int (aka must be positive)\n"
                        << std::flush;
              exit(EXIT_FAILURE);
            }
            ...
            if (i + 1 < argc && '-' != argv[i + 1][0]) { // is it not argv[i+1]
              ...
              try {
                ...
                std::cerr << "invalid argument : provided history length count was out of range (for -d/--depth flag) must "
                             "be unsigned int (aka must be positive)\n"
                          << std::flush;
                exit(EXIT_FAILURE);
              }
