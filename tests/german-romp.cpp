...
// state variables --
class State : public romp::ProtoState
{
public:
  ...
  State()
    : Cache((CACHE){Empty, 0}),  //TODO finish decaling default values
      Chan1()
  {}
...
// Initial States --
// void startstate_init(State S, DATA d)
// {
//   //
...
State* GenStartState()
{
  State* start_states = new State[DATA_NUM];
  for (DATA d=0; d<DATA_NUM, d++)
    start_states[d] = new State(d);
  return start_states;
}
...
//------------------------------------------
bool guard_RecvReqS(State S, NODE i)
{
  return (((CurCmd) == (Empty)) && ((((Chan1).data[((i)) - int_VALUE_C(0)]).Cmd) == (ReqS)));
}

void rule_RecvReqS(State S, NODE i)
{
...
//--------------------
bool guard_RecvReqE(State S, NODE i)
{
  return (((CurCmd) == (Empty)) && ((((Chan1).data[((i)) - int_VALUE_C(0)]).Cmd) == (ReqE)));
}

void rule_RecvReqE(State S, NODE i)
{
...
void rule_SendInv(State S, NODE i)
{
  ...
  // void Chan2 Cmd to Inv, and remove node i from InvSet (State S, invalidation already set out)
  // Gnt* and Inv channel
  // DC: curcmd = E
  // DC: curcmd = S & ExGntd
...
//--------------------
bool guard_RecvInvAck(State S, NODE i)
{
  return (((((Chan3).data[((i)) - int_VALUE_C(0)]).Cmd) == (InvAck)) && ((CurCmd) != (Empty)));
}

void rule_RecvInvAck(State S, NODE i)
// template<class Param>
// void rule_RecvInvAck(State S, Param p)
{
  (((Chan3).data[((p.i)) - int_VALUE_C(0)]).Cmd) = (Empty);
  ((ShrSet).data[((p.i)) - int_VALUE_C(0)]) = (false);
  if ((ExGntd) == (true))
  {
    (ExGntd) = (false);
...
//--------------------
bool guard_SendGntE(State S, NODE i)
{
  return ((((((CurCmd) == (ReqE)) && ((CurPtr) == (i))) && ((((Chan2).data[((i)) - int_VALUE_C(0)]).Cmd) == (Empty))) && ((ExGntd) == (false))) && ({ bool res_ = true; for (State S, int j = 0; j <= (NODE_NUM); j++) { res_ &= (((ShrSet).data[((j)) - int_VALUE_C(0)]) == (false)); } res_; }));
}

void rule_SendGntE(State S, NODE i)
{
...
//--------------------
bool guard_RecvGntS(State S, NODE i)
{
  return ((((Chan2).data[((i)) - int_VALUE_C(0)]).Cmd) == (GntS));
}

void rule_RecvGntS(State S, NODE i)
{
...
//--------------------
bool guard_RecvGntE(State S, NODE i)
{
  return ((((Chan2).data[((i)) - int_VALUE_C(0)]).Cmd) == (GntE));
}

void rule_RecvGntE(State S, NODE i)
{
...
//--------------------
bool guard_Store(State S, NODE i, DATA d)
{
  return ((((Cache).data[((i)) - int_VALUE_C(0)]).State) == (E));
}

void rule_Store(State S, NODE i, DATA d)
{
...
bool CtrlProp(State S, void)
{
  return ({ bool res_ = true; for (State S, int i = 0; i <= (NODE_NUM); i++) { res_ &= ({ bool res_ = true; for (State S, int j = 0; j <= (NODE_NUM); j++) { res_ &= (!((i) != (j)) || ((!((((Cache).data[((i)) - int_VALUE_C(0)]).State) == (E)) || ((((Cache).data[((j)) - int_VALUE_C(0)]).State) == (I))) && (!((((Cache).data[((i)) - int_VALUE_C(0)]).State) == (S)) || (((((Cache).data[((j)) - int_VALUE_C(0)]).State) == (I)) || ((((Cache).data[((j)) - int_VALUE_C(0)]).State) == (S)))))); } res_; }); } res_; });
}

bool DataProp(State S, void)
{
  return ((!((ExGntd) == (false)) || ((MemData) == (AuxData))) && ({ bool res_ = true; for (State S, int i = 0; i <= (NODE_NUM); i++) { res_ &= (!((((Cache).data[((i)) - int_VALUE_C(0)]).State) != (I)) || ((((Cache).data[((i)) - int_VALUE_C(0)]).Data) == (AuxData))); } res_; }));
}


// << =============================== Rule Object Declarations ================================= >> 
...
std::function<bool(State)> INVARIANTS = { CtrlProp, DataProp };
#define INVARIANTS_SIZE 2
std::function<bool(State)> ASSERTIONS = {};
#define ASSERTIONS_SIZE 0
